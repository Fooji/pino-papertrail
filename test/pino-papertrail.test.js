...
function testApiTcp ({ t, connection, echo, prettify = false, expectedMessages }) {
  t.plan(11)
...
test('pino-papertrail api (tcp)', (t) => testApiTcp({
  t,
  connection: 'tcp',
  echo: false,
  prettify: false,
  expectedMessages: { infoMessage: messages.infoMessage, warnMessage: messages.warnMessage }
}))
  .then(() => {
    test('pino-papertrail api (tls)', (t) => testApiTcp({
      t,
      connection: 'tls',
      echo: true,
