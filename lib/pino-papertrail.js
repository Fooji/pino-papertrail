...
function _jsonParser (str) {
  ...
  if (result.err) {
    console.log('There were problems parsing the json...', result.err)
  } else {
    return result.value
  }
}
...
function toPapertrailTcp (options) {
  ...
  console.log('The papertrail options: ', options)
  ...
  function createSocket () {
    ...
    const call = backoff.call((callback) => {
      ...
      socket.on('error', (err) => {
        isConnected = false
        console.log('Error(papertrail): ', err)
        callback(err)
      })
      socket.on('end', () => {
        isConnected = false
  ...
  const writableStream = new stream.Writable({
    write (data, encoding, callback) {
      ...
      console.log('Sending data to papertrail ...', isConnected)
...
module.exports.toSyslog = function (options) {
  console.log('To syslog producer ...')
  const syslogProducer = new glossy.Produce()
  console.log('The options before checking the options [0] ...', options)

  return through2.obj(function transport (data, enc, cb) {
    let message
    console.log('The options before checking the options [1] ...', options)
    if (options['message-only']) {
      message = data.msg
    } else if (options.prettify) {
      const pretty = prettyFactory({ ...options.prettify })
      message = pretty(data)
    } else {
      message = JSON.stringify(data)
    }

    console.log('Before syslog producer ...', data)
